    ...
    <div className="fixed bottom-0 left-0 right-0 pb-6 z-30">
      <div className="mx-auto w-11/12 max-w-md bg-gradient-to-r from-blue-500 to-blue-600 rounded-full py-3 px-4 shadow-lg">
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={() => handleNavigation('/meditation-list', 'Meditation')}
            className={`transition-transform flex flex-col items-center relative ${isActive('/meditation-list') || isActive('/meditation-dashboard') ? 'text-white scale-110' : 'text-white/80 hover:text-white'}`}
          >
            <Briefcase size={24} />
            <span className="text-xs mt-1 font-medium">Meditation</span>
            {(isActive('/meditation-list') || isActive('/meditation-dashboard')) && <div className="absolute -bottom-2 w-12 h-0.5 bg-white rounded-full"></div>}
          </button>
