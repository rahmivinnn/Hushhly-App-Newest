...
import { getMeditationsByCategory } from '@/data/meditationData';
...
const MeditationChat: React.FC<MeditationChatProps> = ({ isOpen, onClose }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await streamAIChat(userMessage.content, history, (token) => {
      ...
      setMessages(prev => prev.map(m => m.id === aiMsgId ? aiResponse : m));
      setIsTyping(false);
        ...
        recognition.onresult = (event) => {
          ...
          setTimeout(() => {
            if (transcript.trim() !== '') {
              ...
              const history = [...messages, userMessage];
              streamAIChat(transcript, history, (token) => {
                setMessages(prev => prev.map(m => m.id === aiMsgId ? { ...m, content: (m.content || '') + token } : m));
              }, () => {
                setIsTyping(false);
              }).catch(() => {
  ...
  const handleSuggestionClick = (suggestion: string) => {
    ...
    const history = [...messages, userMessage];
    streamAIChat(suggestion, history, (token) => {
      setMessages(prev => prev.map(m => m.id === aiMsgId ? { ...m, content: (m.content || '') + token } : m));
    }, () => {
      setIsTyping(false);
    }).catch(() => {
  ...
  const generateAIResponse = (userMessage: string): ChatMessage => {
    ...
    // Quick trigger: start specific meditation by quoted title (e.g., Start "Morning Affirmations")
    const quoted = userMessage.match(/start\s+"([^"]+)"/i);
    if (quoted) {
      setMeditationTitle(quoted[1]);
      messageType = 'meditation';
      responseContent = `Let's start "${quoted[1]}" now. Find a comfortable position, inhale 4, hold 4, exhale 6. I’ll guide you gently.`;
      suggestions = ['Start a 10-minute timer', 'Add background sound', 'Schedule again tomorrow'];
    ...
    if (emotion === 'sad' || emotion === 'anxious') {
      setBearEmotion('calm');
      setBearMessage("I understand how you feel. Let's take a gentle breathing moment together.");
      showHug = true;
      setTimeout(() => {
      ...
      setBearMessage("That's wonderful to hear. Let's strengthen this positive energy.");
    } else if (emotion === 'tired') {
      setBearEmotion('thinking');
      setBearMessage("Okay, let's choose a gentle practice that respects your energy today.");
    } else {
      setBearEmotion('listening');
      setBearMessage("I'm here with you. What do you need most right now?");
    }

    // Prepare dynamic Synced Just Now suggestions
    const syncedToday = getMeditationsByCategory('Synced Just Now').slice(0, 3);
    const syncedSuggestions = syncedToday.map(m => `Start "${m.title}"`);

    // Check for keywords and generate structured English response
    if (lowerMessage.includes('hello') || lowerMessage.includes('hi') || lowerMessage.includes('hey')) {
      responseContent = "I'm here, fully present with you.\nHow is your inner world today? Share what’s on your heart.";
      suggestions = ['I feel anxious', 'I feel tired', 'I feel grateful', 'I need focus', ...syncedSuggestions];
    }
      ...
      responseContent = "I feel your anxiety with you.\nQuick steps to calm the nervous system:\n- Inhale 4 • Hold 4 • Exhale 6 (3 cycles)\n- Place attention on your chest or belly; notice sensations as they arise\nWould you like a gentle 5-minute practice?";
      suggestions = ['Yes, help me breathe', 'Chest/Belly', 'Quick option', 'Schedule a session', ...syncedSuggestions];
      setMeditationTitle('Anxiety Relief');
    }
    else if (lowerMessage.includes('tired') || lowerMessage.includes('exhausted') || lowerMessage.includes('no energy')) {
      responseContent = "I hear your fatigue. Let's choose between two short options:\n- Gentle energy (slow wake, 4–4–6)\n- Sleep preparation (deep relax, long breaths)\nWhich one would you like?";
      suggestions = ['Gentle energy', 'Help me sleep', 'Sleep better', 'Why do I feel tired often?', ...syncedSuggestions];
      setMeditationTitle('Gentle Energy Renewal');
    }
    else if (lowerMessage.includes('sad') || lowerMessage.includes('depressed') || lowerMessage.includes('down')) {
      responseContent = "I'm sitting with you in this feeling.\nLet's create a safe and warm space:\n- Breathing 4–4–6, notice body sensations\n- Soft phrase: ‘I am safe. My feelings are valid.’\nWould you like to practice holding space now?";
      suggestions = ['Yes, stay with me', 'How does meditation help sadness?', 'I want to share more', 'Schedule a session', ...syncedSuggestions];
      setMeditationTitle('Emotional Holding Space');
    }
    else if (lowerMessage.includes('happy') || lowerMessage.includes('grateful') || lowerMessage.includes('good')) {
      responseContent = "Your energy is beautiful. Let's deepen gratitude quickly:\n- 60 seconds: name 3 things you’re grateful for\n- 2 minutes: gratitude breath (inhale 4, hold 4, exhale 6)\nWould you like a short gratitude practice?";
      suggestions = ['Short gratitude practice', 'Write 3 gratitude items', 'Gratitude breathing', ...syncedSuggestions];
      setMeditationTitle('Gratitude Practice');
    }
    else if (lowerMessage.includes('focus') || lowerMessage.includes('concentrate') || lowerMessage.includes('productivity')) {
      responseContent = "Focus can be challenging. Quick steps:\n- 90 seconds: close your eyes, breathe 4–4–6\n- Write one small goal for the next 10 minutes\nWould you like a 5-minute focus reset?";
      suggestions = ['Start 5-minute focus session', 'Write a small goal', 'I need quick help', ...syncedSuggestions];
      setMeditationTitle('Focus Reset');
    }
    else if (lowerMessage.includes('sleep') || lowerMessage.includes('insomnia') || lowerMessage.includes('rest')) {
      responseContent = "For restful sleep, try this short flow:\n- 2 minutes: body scan from head to toes\n- Long breaths 4–7–8 (3 times)\nReady for a sleep preparation meditation?";
      suggestions = ['Yes, guide me to sleep', 'Better sleep tips', 'I need it now', 'Schedule for tonight', ...syncedSuggestions];
      setMeditationTitle('Sleep Preparation');
    }
    else if (lowerMessage.includes('yes') && (lowerMessage.includes('meditation') || lowerMessage.includes('breathe') || lowerMessage.includes('help') || lowerMessage.includes('focus') || lowerMessage.includes('start'))) {
      responseContent = "Let’s begin now.\nFind a comfortable position, arrive fully.\nInhale 4 • Hold 4 • Exhale 6 (3 cycles). Then we continue with gentle guidance.";
      messageType = 'meditation';
      suggestions = [];
    }
    else if (lowerMessage.includes('schedule') || lowerMessage.includes('later') || lowerMessage.includes('reminder')) {
      responseContent = "Happy to schedule. What time feels best?";
      suggestions = ['Tonight', 'Tomorrow morning', 'Custom time', 'Cancel'];
    }
    else if (lowerMessage.includes('evening') || lowerMessage.includes('tonight')) {
      responseContent = "Evening practice is lovely—a gentle transition to rest. Save a quick reflection first?";
      suggestions = ['Yes, save reflection', 'No, just schedule', 'Cancel'];
      setTimeout(() => { setShowScheduleModal(true); }, 1000);
    }
    else if (lowerMessage.includes('morning') || lowerMessage.includes('tomorrow')) {
      responseContent = "Morning meditation sets a warm tone for the day. Save a quick reflection first?";
      suggestions = ['Yes, save reflection', 'No, just schedule', 'Cancel'];
      setTimeout(() => { setShowScheduleModal(true); }, 1000);
    }
    else if (lowerMessage.includes('reflection') || lowerMessage.includes('save')) {
      responseContent = "Your reflection is saved—beautiful to revisit later.";
      suggestions = ['Thank you', 'Start meditation now', 'Explain reflections'];
      setTimeout(() => {
        toast({ title: "Reflection Saved", description: "Your reflection was saved to your journal.", duration: 3000 });
      }, 1000);
      messageType = 'reflection';
    }
    else {
      responseContent = "I’m here for you. Prefer a tailored practice based on how you feel now, or a specific guide?";
      suggestions = ['Start a meditation', 'Help with anxiety', 'Help with focus', 'Help with sleep', ...syncedSuggestions];
    }
  }
