...
// Get category-specific tracks from the comprehensive data
const getCategoryTracks = (category: string) => {
  const meditations = getMeditationsByCategory(category);
  return meditations.slice(0, 10).map(meditation => ({
    title: meditation.title,
    duration: meditation.duration,
    listeners: meditation.listeners,
    image: meditation.image,
    icon: meditation.icon,
    lottieAnimation: meditation.lottieAnimation
  }));
};
        ...
        <div className="w-full">
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 mb-6 shadow-lg">
            <div className="flex items-center mb-4">
              <div className={`w-20 h-20 rounded-xl bg-gradient-to-br ${gradientClass} flex items-center justify-center mr-4 flex-shrink-0 shadow-md ${isPlaying ? 'animate-pulse-medium' : ''} relative overflow-hidden`}>
                <span className="text-5xl">{selectedTrack.icon}</span>
                {/* Lottie Animation Overlay */}
                {selectedTrack.lottieAnimation && (
                  <div className="absolute inset-0 flex items-center justify-center">
                    <LottieAnimation
                      animationData={getLottieAnimation(selectedTrack.lottieAnimation)}
