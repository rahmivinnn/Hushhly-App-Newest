...
import { getStoriesByCategory } from '@/data/sleepStories';
...
const Stories = () => {
  ...
  // Realtime stories per category (falls back to static when Firebase not configured)
  const { data: featuredData } = useRealtimeStories('featured');
  const { data: shortData } = useRealtimeStories('short');
  const { data: longData } = useRealtimeStories('long');
  const { data: familyData } = useRealtimeStories('family');
  ...
  }, [searchTerm, activeFilter, featuredData, shortData, longData, familyData]);
